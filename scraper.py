...
def save_data(data, columns, data_set_name, file_name):
    ...
    print(data_set_name + ' saved as: ' + os.path.dirname(os.path.abspath(__file__)) + '/' + file_name + '-' + data_set_name + '.csv')
...
def find_col(soup, team):
    try:
        if team:
            columns = list(filter(None, soup.find('thead').get_text().split('\n')))
            return get_team_data(soup, columns)
        else:
            columns = re.sub(r'\n[\s]*', '\n',
                             soup.find('thead').get_text()).strip().split('\n')
...
def get_years(soup):
    ...
    years = [year.get_text() for year in soup.select('select#season option')]
    if years:
        years[-1] = 'all-time'
    return years
...
def get_year_stats():
    ...
    try:
        ...
        if page.status_code == 200:
            ...
            print(f"Fetched stats_url: {stats_url}")  
            print(f"Fetched stats_title: {stats_title}")  
...
def main():
    ...
    if not years or not stats_url or not stats_title:
        print("Failed to fetch years or stats.")
        return

    years_q, stats_q = prepare_question(years, stats_title, stats_url)
    if not years_q or not stats_q:
        print("No years or stats available for selection.")
        return

    years, stats = user_input(years_q, stats_q)
    if not years or not stats:
        print("No years or stats selected.")
        return
